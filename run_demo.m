...
%  Run this to verify the GEV beamformer works
...
%% Setup
fs = 16000; c = 343; dur = 3; N = dur*fs; t = (0:N-1)'/fs;
num_mics = 6; r = 0.05;
mic_pos = r * [cosd((0:5)*60)', sind((0:5)*60)'];

%% Create Test Signals
% Target: Clear speech-like signal at 0° (harmonics 150-1200 Hz)
f0 = 150;
target = zeros(N, 1);
for h = 1:8
    target = target + sin(2*pi*f0*h*t) / sqrt(h);
end
target = target .* (0.6 + 0.4*sin(2*pi*2.5*t));  % Natural AM
target = target / max(abs(target));

% Interference: Strong distinct noise at 90° (higher freq band)
interf = bandpass(randn(N, 1), [800 2500], fs);
interf = interf / max(abs(interf));

fprintf('Target: Speech harmonics at 0°\n');
fprintf('Interference: Band noise (800-2500Hz) at 90°\n');
...
mic_sig = mic_sig / max(abs(mic_sig(:)));
...
%% GEV Beamforming (finer resolution = better accuracy)
fov = [-15 15]; 
angles = -180:10:170;  % 10° resolution for better accuracy
reg = 1e-6;

fprintf('Processing %d frequency bins...\n', length(bins));
...
y = y / max(abs(y));  % Normalize to full scale
ref = mic_sig(:,1) / max(abs(mic_sig(:,1)));

%% Beam Pattern at 1kHz
...
subplot(1,3,1);
spectrogram(ref,hann(256),192,256,fs,'yaxis'); 
title('INPUT (Target + Interference)'); ylim([0 4]); colorbar;

subplot(1,3,2);
spectrogram(y,hann(256),192,256,fs,'yaxis'); 
title('OUTPUT (Interference Suppressed)'); ylim([0 4]); colorbar;

subplot(1,3,3);
plot(pa, bp_dB, 'b-', 'LineWidth', 2); hold on;
xline(0, 'g--', 'LineWidth', 2);
xline(90, 'r--', 'LineWidth', 2);
fill([fov(1) fov(2) fov(2) fov(1)], [-50 -50 5 5], 'g', 'FaceAlpha', 0.15, 'EdgeColor', 'none');
xlabel('Angle (°)'); ylabel('Gain (dB)');
title(sprintf('Beam Pattern (%.1f dB suppression)', suppression));
xlim([-180 180]); ylim([-35 5]); grid on;
legend('Pattern', 'Target 0°', 'Interf 90°', 'FOV');

sgtitle('Audiovisual Zooming - GEV Beamformer Demo', 'FontWeight', 'bold');

%% Play Audio
fprintf('\nPlaying INPUT...\n');
soundsc(ref, fs); pause(dur + 0.5);
fprintf('Playing OUTPUT (louder, cleaner)...\n');
soundsc(y, fs);
